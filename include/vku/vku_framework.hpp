...
#define VK_KHR_SURFACE_NAME "VK_KHR_surface"
#define VK_LAYER_LUNARG_STD_VALIDATION_NAME "VK_LAYER_LUNARG_standard_validation"

#ifdef _WIN32
#define VK_USE_PLATFORM_WIN32_KHR
#define GLFW_EXPOSE_NATIVE_WIN32
#define VKU_SURFACE_NAME "VK_KHR_win32_surface"
#pragma warning(disable : 4005)
#else
#define VK_USE_PLATFORM_XLIB_KHR
#define GLFW_EXPOSE_NATIVE_X11
#define VKU_SURFACE_NAME "VK_KHR_xlib_surface"
#endif
...
namespace vku {
	...
	struct FrameworkOptions {
		bool enableStdValidation = true;
		bool gammaCorrection = true;
		std::vector<std::string> requestedLayers;
		std::vector<std::string> requestedExtensions;
		//blabla: add everything configurable
	};
	...
	class Framework {
	public:
		//no default constructor for the sake of looking important?
		Framework() = delete;

		Framework(const std::string &name):
			Framework(name, [](auto& frameworkOptions) {}) {}

		// Construct a framework containing the instance, a device and one or more queues.
		Framework(const std::string &name, const std::function<void(FrameworkOptions& frameworkOptions)>& fwOptionsFunc) {
			FrameworkOptions fwOptions;
			fwOptionsFunc(fwOptions);
			*this = std::move(Framework(name, fwOptions));
		}

		Framework(const std::string &name, const FrameworkOptions& frameworkOptions) {
			std::vector<const char *> layers;
			std::vector<const char *> instance_extensions;

			for (const auto rExt : frameworkOptions.requestedExtensions) {
				instance_extensions.push_back(rExt.data());
			}

			for (const auto rLay : frameworkOptions.requestedLayers) {
				layers.push_back(rLay.data());
			}

			if (frameworkOptions.enableStdValidation)
				layers.push_back(VK_LAYER_LUNARG_STD_VALIDATION_NAME);
			
			instance_extensions.push_back(VK_EXT_DEBUG_REPORT_EXTENSION_NAME);
			instance_extensions.push_back(VKU_SURFACE_NAME);
			instance_extensions.push_back(VK_KHR_SURFACE_NAME);

			// Make sure requested layers or extensions only present in the vector once
			...
			std::sort(instance_extensions.begin(), instance_extensions.end());
